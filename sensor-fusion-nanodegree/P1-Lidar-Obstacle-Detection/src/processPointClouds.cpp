...
ProcessPointClouds<PointT>::RansacPlaneSegment(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceTol)
{
  ...
  auto allPoints = cloud->points;

  Ransac<PointT> segRansac(maxIterations, distanceTol, numPoints);

  // get inliers from local-RANSAC implementation
  ...
  typename pcl::PointCloud<PointT>::Ptr outPlane(new pcl::PointCloud<PointT>());
  typename pcl::PointCloud<PointT>::Ptr inPlane(new pcl::PointCloud<PointT>());

  for (int i=0; i<numPoints; i++) {
      PointT pt = allPoints[i];
      if (inliersResult.count(i)) {
          outPlane->points.push_back(pt);
        }
      else {
          inPlane->points.push_back(pt);
        }
    }

  return std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>(inPlane, outPlane);
}
